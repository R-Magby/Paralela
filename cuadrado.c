...
void guardar_datos(float* phi,int M, int N){
    ...
    FILE *fp = fopen("L_infinito.dat", "w");
    for (int i=0; i<M; i++) {
        for (int j=0; j<N; j++) {
            fprintf(fp, "%g\n", phi[i*N+j]);
        }
...
static void llenar_rho(float* rho, int filas,int columnas, float L, float dx,float dy ,float l){
...
//vertical
            float xd = j*dx-L;
            float yd = i*dy-L/2.0;
            float upd= (yd+l)+sqrt((xd)*(xd)+(yd+l)*(yd+l)+0.5);
            float downd=(yd-l)+sqrt((xd)*(xd)+(yd-l)*(yd-l)+0.5);

            float xI = j*dx;
            float yI = i*dy-L/2.0;
            float upI= (yI+l)+sqrt((xI)*(xI)+(yI+l)*(yI+l)+0.5);
            float downI=(yI-l)+sqrt((xI)*(xI)+(yI-l)*(yI-l)+0.5);
//Horizontal

            float xU = j*dx-L/2.0;
            float yU = i*dy;
            float upU= (xU+l)+sqrt((xU+l)*(xU+l)+yU*yU+0.5);
            float downU=(xU-l)+sqrt((xU-l)*(xU-l)+yU*yU+0.5);
        
            float xA = j*dx-L/2.0;
            float yA = i*dy-L;
            float upA= (xA+l)+sqrt((xA+l)*(xA+l)+yA*yA+0.5);
            float downA=(xA-l)+sqrt((xA-l)*(xA-l)+yA*yA+0.5);

            rho[i*columnas+j] = log(upd/downd)+log(upI/downI)-log(upU/downU)+log(upA/downA);
        }
    }
}
...
//por los demas procesos
    if(myID==0){
        ...
        llenar_rho(rho, rows,colums, L, dx, dy,a);
    }

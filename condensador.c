...
void guardar_datos(float* phi,int M, int N){
    ...
    FILE *fp = fopen("condensador.dat", "w");
    for (int i=0; i<M; i++) {
        for (int j=0; j<N; j++) {
            fprintf(fp, "%g\n", phi[i*N+j]);
        }
...
static void llenar_rho(float* rho, int filas,int columnas, float L, float dx,float dy ,float l){
    ...
    for (int i = 0; i < filas; i++){
        for (int j = 0; j < columnas; j++){
            float x1 = j*dx-L/2;
            float y1 = i*dy-3.0*L/4.0;
            float up1= (x1+l)+sqrt((x1+l)*(x1+l)+y1*y1+1);
            float down1=(x1-l)+sqrt((x1-l)*(x1-l)+y1*y1+1);

            float x2 = j*dx-L/2;
            float y2 = i*dy-L/4.0;
            float up2= (x2+l)+sqrt((x2+l)*(x2+l)+y2*y2+1);
            float down2=(x2-l)+sqrt((x2-l)*(x2-l)+y2*y2+1);
            rho[i*columnas+j] = log(up1/down1)-log(up2/down2);
        }
    }
}

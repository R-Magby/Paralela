...
static void llenar_rho(float* rho, int filas,int columnas, float L, float dx,float dy ,float l){
    ...
    for (int i = 0; i < filas; i++){
        for (int j = 0; j < columnas; j++){
            float x1 = j*dx-L/2.0;
            float y1 = i*dy-L/4.0;
            float up1= (y1+l)+sqrt((x1)*(x1)+(y1+l)*(y1+l)+0.5);
            float down1=(y1-l)+sqrt((x1)*(x1)+(y1-l)*(y1-l)+0.5);

            float x2 = j*dx-L/4.0;
            float y2 = i*dy-L/2.0;
            float up2= (x2+l)+sqrt((x2+l)*(x2+l)+y2*y2+0.5);
            float down2=(x2-l)+sqrt((x2-l)*(x2-l)+y2*y2+0.5);
            rho[i*columnas+j] = log(1000000000/down1)+0.75*log(1000000000/down2);
        }
    }
}
